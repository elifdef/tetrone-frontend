...
export default function PostPoll({ poll, postId, isOwner })
{
    ...
    {
        if (idsToSubmit.length === 0 || isClosed)
        {
            return;
        }
        setIsLoading(true);
        ...
        if (res)
        {
            ...
            if (res.poll.quiz_data)
            {
                setQuizData(res.poll.quiz_data);
            }
        }
        else
        {
    ...
    const handleOptionClick = (optionId) =>
    {
        ...
        if (votedOptionIds.length > 0 && !poll.can_change_vote)
        {
            return;
        }
        ...
        {
            if (draftOptionIds.includes(optionId))
            {
                return;
            }
        ...
        <div className="tetrone-poll">
                        ...
                        <div key={ option.id } className="tetrone-poll-option-wrapper">
                            <div onClick={ () => handleOptionClick(option.id) }
                                ...
                                <div className="tetrone-poll-text">
                                    { poll.is_multiple_choice && <input type="checkbox" checked={ isSelected } readOnly
            ...
            { poll.is_multiple_choice && hasDraftChanges && !isClosed && (
                <div className="tetrone-poll-actions tetrone-poll-actions-start">
                    <Button
                        onClick={ () => submitVote(draftOptionIds) }
                        disabled={ isLoading || draftOptionIds.length === 0 }>
                        { t('action.vote') }
                    </Button>
                    { votedOptionIds.length > 0 &&
                        <button className="tetrone-btn tetrone-btn-cancel" onClick={ cancelDraft }
                                disabled={ isLoading }>{ t('action.cancel') }</button> }
                </div>
            ) }
            ...
            <div className="tetrone-poll-meta tetrone-poll-meta-spaced">
                <div className="tetrone-poll-meta-left">
                    ...
                    { poll.is_anonymous && <><span
                        className="dot">•</span><span>{ t('post.anonymous_poll') }</span></> }
                    { isClosed && <><span className="dot">•</span><span
                        className="tetrone-poll-closed-badge">{ t('poll.closed_badge') }</span></> }
                </div>

                { isOwner && !isClosed && (
                    <button className="tetrone-poll-close-btn"
                            onClick={ handleClosePollClick }>{ t('action.close') }</button>
                ) }
            </div>

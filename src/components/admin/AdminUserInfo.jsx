...
export default function AdminUserInfo() {
    ...
    if (isLoading) return <div className="socnet-empty-state">{t('common.loading')}</div>;
    if (!user) return <div className="socnet-empty-state with-card">{t('admin.user_info.not_found')}</div>;

    return (
        <div className="admin-user-wrapper">
            <div className="socnet-card-wrapper admin-user-card-header">
                <h2 className="admin-user-title">
                    <a href={`/${user.username}`} target="_blank" rel="noreferrer" className="socnet-link">
                        @{user.username}
                    </a>
                </h2>

                <div className="admin-user-tabs socnet-tabs">
                    <button className={`socnet-tab ${activeTab === 'info' ? 'active' : ''}`} onClick={() => setActiveTab('info')}>
                        {t('common.profile', 'Профіль')}
                    </button>
                    <button className={`socnet-tab ${activeTab === 'posts' ? 'active' : ''}`} onClick={() => setActiveTab('posts')}>
                        {t('common.posts', 'Пости')} ({user.posts_count || 0})
                    </button>
                    <button className={`socnet-tab ${activeTab === 'comments' ? 'active' : ''}`} onClick={() => setActiveTab('comments')}>
                        {t('common.comments', 'Коментарі')} ({user.comments_count || 0})
                    </button>
                    <button className={`socnet-tab ${activeTab === 'likes' ? 'active' : ''}`} onClick={() => setActiveTab('likes')}>
                        {t('common.likes', 'Лайки')} ({user.likes_count || 0})
                    </button>
                    <button className={`socnet-tab ${activeTab === 'sessions' ? 'active' : ''}`} onClick={() => setActiveTab('sessions')}>
...
const TabInfo = ({ user, formatDate, handleToggleAction, t }) => (
    <>
        ...
        <div className="socnet-card-wrapper admin-table-card mt-3">
...
const TabPosts = ({ username, t }) => {
    ...
    return (
        <InfiniteScrollList
            ...
            emptyState={<div className="socnet-empty-state">{t('common.no_more_data')}</div>}
        >
...
const TabComments = ({ username, t }) => {
    ...
    return (
        ...
        >
            {comments.map(c => (
                <div key={c.id} className="socnet-card-wrapper" style={{ padding: '15px', marginBottom: '10px' }}>
...
const TabSessions = ({ username, t, formatDate, openConfirm }) => {
    ...
    if (sessions.length === 0) return <div className="socnet-empty-state">{t('admin.user_info.history_empty')}</div>;

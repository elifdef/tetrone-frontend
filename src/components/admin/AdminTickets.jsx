...
const AdminTicketDetail = ({ticket, onBack, onTicketUpdated}) =>
{
    ...
    const isPartnership = ticket.category === 'general' && ticket.subcategory === 'partnership';

    const canChangeRole = isAdmin && isPartnership && ticket.user && (currentAdmin.role === userRole.Owner || ticket.user.role < currentAdmin.role);
        ...
        <div className="flex flex-col gap-[15px] font-tahoma text-[11px]">
            <div className="bg-bg-box border border-border">
                <div className="bg-header-bg p-[6px_10px] font-bold text-[11px] border-b border-border text-text-main flex justify-between items-center">
                    ...
                    <Button variant="secondary" onClick={onBack}>{t('action.go_back')}</Button>
                </div>
                <div className="p-[10px_15px]">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="mb-[4px]">
                                ...
                                <strong className="text-theme-link">{t(`support.cat_${ticket.category}`)}</strong>
                                {ticket.subcategory && <span className="text-text-muted ml-[5px]">/ {t(`support.subcat_${ticket.subcategory}`)}</span>}
                            </div>
                        ...
                        {canChangeRole && (
                            <Button variant="warning" onClick={() => setRoleModalUser(ticket.user)} className="shrink-0 ml-[10px]">
                    ...
                    {(ticket.meta_data?.user_agent || ticket.meta_data?.steps_to_reproduce) && (
                        <div className="mt-[10px] pt-[10px] border-t border-dotted border-border">
                            {ticket.meta_data?.user_agent && <div className="mb-[4px]"><span className="text-text-muted">{ticket.meta_data.user_agent}</span></div>}
                            {ticket.meta_data?.steps_to_reproduce && (
                                <div className="mt-[8px] mb-[4px]">
                                    <div className="mb-[4px] text-text-muted font-bold">{t('support.field_steps')}:</div>
                                    <div className="italic bg-bg-page border-l-[2px] border-theme-link p-[5px_8px] text-text-muted">{ticket.meta_data.steps_to_reproduce}</div>
                                </div>
                            )}
                        </div>
                    )}
            ...
            <div className="flex flex-col gap-[10px]">
                ...
                {
                    const isMessageAdmin = msg.user?.username !== ticket.user?.username;
                    ...
                    if (isInternal)
                    {
                        borderClass = "border-[#d39e00]";
                        headerBgClass = "bg-[#fff9e6]";
                        headerTextClass = "text-[#8a6d3b]";
                        bodyBgClass = "bg-[#fffdf5]";
                        bodyTextClass = "text-[#333333]";
                    } else if (isMessageAdmin)
                    {
                        borderClass = "border-[#a3c2e0]";
                        headerBgClass = "bg-[#eef5fc]";
                        bodyBgClass = "bg-[#f7fafd]";
                    }
                        ...
                        <div key={msg.id} className={`border text-[12px] ${borderClass} ${bodyBgClass}`}>
                            <div className={`border-b p-[6px_10px] flex justify-between items-center ${headerBgClass} ${borderClass}`}>
                                <span className={`font-bold ${isInternal ? headerTextClass : 'text-theme-link'}`}>
                                    ...
                                    {isInternal && <span className="text-[#cc0000] font-bold ml-[5px] text-[10px]">[{t('admin.support.internal_note')}]</span>}
                                </span>
                                <span className="text-[10px] text-text-muted">{formatDate(msg.created_at)}</span>
                            </div>
                            <div className={`p-[10px] leading-[1.4] whitespace-pre-wrap ${bodyTextClass}`}>{msg.message}</div>
                            {msg.attachments?.length > 0 && (
                                <div className="border-t border-dotted border-border bg-bg-page p-[8px_10px] flex flex-col gap-[5px]">
                                    {msg.attachments.map(att => <a key={att.id} href={att.url || att.file_url} target="_blank" rel="noreferrer" className="text-[11px] text-theme-link font-bold hover:underline no-underline bg-transparent">{att.file_name}</a>)}
                                </div>
                            )}
                        </div>
                    );
            ...
            {!isClosed ? (
                <div className="bg-bg-box border border-border">
                    <div className="bg-header-bg p-[6px_10px] font-bold text-[11px] border-b border-border text-text-main">{t('action.reply')}</div>
                    <form onSubmit={handleReply} className="p-[10px]">
                        <textarea className="w-full min-h-[100px] mb-[10px] resize-y border border-input-border bg-input-bg p-[6px] text-[11px] text-text-main focus:outline-none focus:border-border shadow-[inset_1px_1px_2px_rgba(0,0,0,0.1)]" value={replyText}
                                  onChange={(e) => setReplyText(e.target.value)} placeholder={t('admin.support.reply_placeholder')} required/>
                        <div className="flex justify-between items-center border-t border-dotted border-border pt-[10px]">
                            <label className="flex items-center gap-[5px] font-bold text-[#cc0000] cursor-pointer">
                                <input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} className="cursor-pointer"/> {t('admin.support.internal_note_desc')}
                            </label>
                            <div className="flex gap-[8px]">
                                <Button type="button" variant="reject" onClick={handleClose}>{t('action.close')}</Button>
                                <Button type="submit" variant="primary" disabled={replying || !replyText.trim()}>{replying ? t('action.saving') : t('action.send')}</Button>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div className="p-[15px] text-center text-text-muted border border-border bg-bg-box">
                    ...
                    {ticket.closed_by && <div className="mt-[5px] text-[10px]">{t('admin.support.closed_by')} <Link to={`/${ticket.closed_by.username}`} className="text-theme-link hover:underline bg-transparent no-underline">@{ticket.closed_by.username}</Link></div>}
                </div>
            )}
            ...
            <ChangeRoleModal
                ...
                onSuccess={() => onTicketUpdated(ticket.id)}
            />
        </div>
    );
};
...
export default function AdminTickets()
{
    ...
    {
        setLoading(true);
        AdminService.getTickets(filters)
        .onSuccess((res) =>
        {
    ...
    const handleSelectTicket = (id) =>
    {
        ...
        .onSuccess((res) => setSelectedTicket(res.ticket))
        .onError((err) => notifyError(t(`api.error.${err.code || 'ERR_UNKNOWN'}`)))
    ...
    if (selectedTicket)
    {
        return <AdminTicketDetail ticket={selectedTicket} onBack={() => setSelectedTicket(null)} onTicketUpdated={handleSelectTicket}/>;
    }

    return (
        <div className="flex flex-col gap-[15px] font-tahoma text-[11px]">
            <div className="flex flex-wrap items-center gap-[10px] bg-bg-box p-[15px] border border-border">
                <input
                    type="text"
                    className="flex-1 min-w-[200px] border border-input-border bg-input-bg px-[8px] h-[28px] box-border text-[11px] text-text-main focus:outline-none focus:border-border transition-colors shadow-[inset_1px_1px_2px_rgba(0,0,0,0.1)]"

...
export default function EditPostModal({isOpen, onClose, post, onSaveSuccess})
{
    ...
    const footerButtons = activeTab === 'content' ? (
        <div className="tetrone-edit-footer-split">
            <AttachBar onFileSelect={handleFileSelect}/>
            <div className="tetrone-edit-footer-actions">
    ...
    return (
        <Modal
            ...
            dialogClassName={`tetrone-edit-modal-wide ${isDragging ? 'drag-active' : ''}`}
            footer={footerButtons}
        >
            <div className="tetrone-edit-tabs">
                <button
                    className={`tetrone-edit-tab ${activeTab === 'content' ? 'active' : ''}`}
                    onClick={() => setActiveTab('content')}
                >
                    <EditIcon width={12} height={12} className="tetrone-tab-icon"/>
                    {t('action.edit')}
                </button>
                <button
                    className={`tetrone-edit-tab ${activeTab === 'settings' ? 'active' : ''}`}
                    onClick={() => setActiveTab('settings')}
                >
                    <SettingsIcon width={12} height={12} className="tetrone-tab-icon"/>
                    {t('common.settings')}
                </button>
            </div>
                        ...
                        {!reactionsDisabled && (
                            <>
                                <div className="tetrone-form-group tetrone-mt-15">
                                    <label className="tetrone-edit-label">{t('post.max_unique_stickers')}</label>
                                    <input
                                        type="number"
                                        className="tetrone-edit-number-input"
                                        min="1" max="50"
                                        value={maxLimit}
                                ...
                                <div className="tetrone-form-group tetrone-mt-15">
                                    <label className="tetrone-edit-label">{t('post.pack_selection_title')}</label>
                                    ...
                                    <div className="tetrone-search-input-wrapper tetrone-mb-15">
                                        <SearchIcon className="tetrone-search-icon" width={14} height={14}/>
                                        <input
                                            type="text"

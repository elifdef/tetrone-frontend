...
export default function AttachBar({ onFileSelect }) {
    ...
    return (
        <div className="tetrone-attach-buttons">
            <label className="tetrone-attach-btn" title={t('wall.attach_photo')} style={{ cursor: 'pointer' }}>
                <input type="file" multiple style={{ display: 'none' }} onChange={onFileSelect} accept="image/*" />
                <ImageAttach width={20} height={20} />
            </label>
            
            <label className="tetrone-attach-btn" title={t('wall.attach_video')} style={{ cursor: 'pointer' }}>
                <input type="file" multiple style={{ display: 'none' }} onChange={onFileSelect} accept="video/*" />
                <VideoAttach width={20} height={20} />
            </label>
            
            <label className="tetrone-attach-btn" title={t('wall.attach_audio')} style={{ cursor: 'pointer' }}>
                <input type="file" multiple style={{ display: 'none' }} onChange={onFileSelect} accept="audio/*" />
                <AudioAttach width={20} height={20} />
            </label>
            
            <label className="tetrone-attach-btn" title={t('wall.attach_document')} style={{ cursor: 'pointer' }}>

...
// ДОДАТИ ЦІ ДВА ІМПОРТИ ДЛЯ ЧЕКБОКСІВ
    ...
    return useMemo(() =>
    {
        ...
        if (preset === 'post')
        {
            extensions.push(
                ...
                // ТУТ ПІДКЛЮЧАЄМО ПЛАГІНИ ДЛЯ ЧЕКБОКСІВ
                TaskList.configure(),
                TaskItem.configure({
                    nested: true,
                }),
                ...
                // Також важливо додати taskList і taskItem у UniqueId, щоб не було багів з ID
                UniqueId.configure({types: ['heading', 'paragraph', 'blockquote', 'codeBlock', 'bulletList', 'orderedList', 'listItem', 'taskList', 'taskItem']})
            );
        }

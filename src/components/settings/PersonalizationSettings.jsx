...
export default function PersonalizationSettings()
{
    ...
    console.log(settings.banner_color);

    const [bannerFile, setBannerFile] = useState(null);
    const [previewBannerImage, setPreviewBannerImage] = useState(initialPersonalization.banner_image);

    const [deg, from, to] = initialPersonalization.banner_color.replace(/linear-gradient\(|\)/g, '').split(',').map(s => s.trim());


    const [grad, setGrad] = useState({ deg: deg, c1: from, c2: to });
                        ...
                        <div className="tetrone-gradient-builder" style={ { marginTop: '15px' } }>
                            ...
                            <div className="tetrone-gradient-controls">
                                <div className="tetrone-gradient-colors">
                                    <PopoverPicker color={ grad.c1 }
                                                   onChange={ (c) => updateGradient(grad.deg, c, grad.c2) } t={ t }/>
                                    <span className="tetrone-gradient-icon">→</span>
                                    <PopoverPicker color={ grad.c2 }
                                                   onChange={ (c) => updateGradient(grad.deg, grad.c1, c) } t={ t }/>
                                </div>
